...
(function(){
  ...
  const SL_CHIP = { ON_PACE:'On pace', AHEAD:'Ahead', BEHIND:'Behind', CAPPED:'At daily limit', DEGRADED:'Degraded', NOT_ACTIVE:'Not active', ACTIVE:'Active', DISABLED:'Off',
    PAUSED_UNTIL_RESET:'Daily limit hit', UNKNOWN:'Unknown', UNDER:'Under budget', OVER:'Over budget' };
  ...
  function slChannels(c){
    ...
    const email = card('Email', 'var(--ch-email)', e.status,
      '<div class="big">'+drillBtn('emails','<span class="sl-num">'+sv(e.sent,'channels.email.sent')+'</span>','Read today\'s emails')+'<small>of '+sv(e.safeCapacity,'channels.email.safeCapacity')+' safe today</small></div>',
      ...
      +(slKnown(e.sent)&&e.sent>0?'<div class="sl-mini">'+drillBtn('emails','<span class="sl-open">Read today\'s emails ›</span>','Read today\'s emails')+'</div>':'')
      +'<div class="sl-mini">REP campaign only. All Staffify email today: '+sv(tot.sent,'channels.email.staffifyTotal.sent')+' sent of '+sv(tot.safeCapacity,'channels.email.staffifyTotal.safeCapacity')+' safe. Open tracking is off, so replies are the read signal.</div>');
    const pv = k.provider || {};
    const credits = slKnown(pv.creditsUsd) ? '$'+pv.creditsUsd.toFixed(2) : null;
    const pace = p.state==='PAUSED_UNTIL_RESET'
      ? 'Bland allows '+esc(pv.dailyCallLimit||k.dailyAllowance||100)+' calls a day on the '+esc(pv.plan||'current plan')+' plan'
        +(slKnown(pv.usedInProviderDay)&&pv.providerDayFirstEt?', and '+esc(pv.usedInProviderDay)+' went out between '+esc(String(pv.providerDayFirstEt).replace(/ ET$/,''))+' and '+esc(pv.providerDayLastEt)+' (Bland\'s day starts at '+esc(pv.providerDayStartedEt||'8:00 PM ET')+')':'')
        +'. Calling resumes at '+esc(p.resetsAtEt||'the reset')+'.'
    ...
    const calls = card('Calls', 'var(--ch-call)', p.state||k.status,
      ...
      (credits?'<div class="sl-credit">Bland credits <b>'+esc(credits)+'</b> <span class="sl-dim">available. Credits pay for calls; the daily limit comes from the plan.</span></div>':(pv.creditsUsd==='UNKNOWN'?'<div class="sl-credit">Bland credits '+slUnk('channels.calls.provider.creditsUsd')+'</div>':''))
      +(slKnown(k.attempts)&&k.attempts===0?'':'<div class="sl-rates"><div><b>'+spct(k.answerRatePct,'channels.calls.answerRatePct')+'</b>answered by a person</div><div><b>'+spct(k.substantiveRatePct,'channels.calls.substantiveRatePct')+'</b>real conversation</div></div>')
    ...
    // Consent-based texting: only people who asked for or agreed to a text, from the Staffify automation line.
    const smsBig = m.status==='ACTIVE'
      ? '<div class="big"><span class="sl-num">'+sv(m.sent,'channels.sms.sent')+'</span><small>sent today</small></div>'
      : '<div class="big"><span class="sl-num" style="font-size:20px">'+(m.status==='DEGRADED'?'Degraded':m.status==='UNKNOWN'?'Unknown':'Off')+'</span></div>';
    const sms = card('Texts', 'var(--ch-sms)', m.status, smsBig,
      (m.status!=='ACTIVE' && m.reason ? '<div class="sl-note">'+esc(m.reason)+'.</div>' : '')
      +kv([['Consent verified',sv(m.consentVerified,'channels.sms.consentVerified')],['Eligible now',sv(m.eligible,'channels.sms.eligible')],
          ['Sent',sv(m.totals&&m.totals.sent,'channels.sms.totals.sent')],['Delivered',sv(m.delivered,'channels.sms.delivered')],
          ['Replied',sv(m.replied,'channels.sms.replied')],['Positive',sv(m.positive,'channels.sms.positive')],
          ['Opted out',sv(m.optedOut,'channels.sms.optedOut')],['To Madison',sv(m.madisonHandoffs,'channels.sms.madisonHandoffs')]])
      +'<div class="sl-mini">Consent-based only'+(m.automationNumber?', from '+esc(m.automationNumber):'')+'. Totals since launch.'
      +(slKnown(m.needsReview)&&m.needsReview>0?' '+esc(m.needsReview)+' waiting for consent review.':'')+' Impact when off: '+esc(m.impact||'email and calling unaffected')+'.</div>');
  ...
  function slNewImport(n){
    ...
    q('#sl-newimport').innerHTML='<h3>'+esc(n.label||'The new list')+' '+drillBtn('newImport','<small>'+sv(n.owners,'newImport.owners')+' owners ›</small>','Owners in the new list')+'</h3>'
      ...
      +row('Can call',n.callEligible,'callEligible')+row('Agreed to texts',n.smsEligible,'smsEligible')+row('In email sequence',n.instantlyEnrolled,'instantlyEnrolled')+row('Touched',n.touched,'touched')+'</div>';
  }
  ...
  function slFeed(items){
    const list = items.slice(0,25).map(a=>{
      const inner='<time datetime="'+esc(a.at)+'">'+esc(etTime(a.at))+'</time><div><div class="what"><i></i>'+esc(a.label||SL_KIND[a.kind]||a.kind)+(a.emailId?' <span class="sl-open">Read ›</span>':'')+'</div>'
        +'<div class="who" title="'+esc([a.who,a.detail].filter(Boolean).join(' · '))+'">'+esc(a.who||'')+(a.detail?' · '+esc(a.detail):'')+'</div></div>';
      return '<li class="fk-'+esc(a.kind)+'">'+(a.emailId?'<button type="button" class="sl-link sl-feedbtn" data-drill="emails" data-id="'+esc(a.emailId)+'" aria-label="Read the email to '+esc(a.who||'')+'">'+inner+'</button>':inner)+'</li>';
    }).join('');
  ...
  function emailBlock(pv, open){
    return '<details class="sl-mail"'+(open?' open':'')+'><summary>Read the email</summary>'
      +'<div class="sl-mailhead"><div><span class="sl-dim">Subject</span> '+esc(pv.subject||'(no subject)')+'</div><div><span class="sl-dim">From</span> '+esc(pv.from||'')+'</div><div><span class="sl-dim">To</span> '+esc(pv.to||'')+'</div></div>'
      +'<div class="sl-mailbody">'+esc(pv.text||'(empty)')+'</div></details>';
  }
  async function slDrill(name, id){
    ...
    if(typeof dlg.showModal==='function' && !dlg.open) dlg.showModal();
    try{
      const r=await fetch('/api/sales-live/?drill='+encodeURIComponent(name)+(id?'&id='+encodeURIComponent(id):''),{cache:'no-store',credentials:'same-origin'});
      ...
      body.innerHTML = (d.rows||[]).length ? d.rows.map(x=>'<div class="r"><b>'+esc(x.who||x.company||'Unnamed')+(x.who&&x.company?' <span class="sl-dim">· '+esc(x.company)+'</span>':'')+'</b>'
          ...
          +(x.detail||safe(x.link)?'<div class="d">'+esc(x.detail||'')+(safe(x.link)?' <a href="'+esc(safe(x.link))+'" target="_blank" rel="noopener noreferrer">'+(name==='emails'?'HubSpot ›':'Open ›')+'</a>':'')+'</div>':'')
          +(x.preview?'<div class="d">'+emailBlock(x.preview, !!id)+'</div>':'')+'</div>').join('')
  ...
  function onClick(e){
    ...
    const b=e.target.closest('[data-drill]'); if(b){ e.preventDefault(); slDrill(b.dataset.drill, b.dataset.id||''); return; }
